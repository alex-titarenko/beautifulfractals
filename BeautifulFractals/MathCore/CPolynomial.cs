...
namespace TAlex.BeautifulFractals.MathCore
{
    ...
    public class CPolynomial : ICloneable, IFormattable, IXmlSerializable
    {
        ...
        {
            s = s.Trim();
            const string polyPattern = @"( (?<sign>[-+])? \(? (?<coef>([-+]?[0-9.,]+i?)*) \)? \*? (?<var>[_a-zA-Z][a-zA-Z0-9]*)? (\^(?<exp>[0-9]+))? )";

            MatchCollection matches = System.Text.RegularExpressions.Regex.Matches(s, polyPattern, RegexOptions.IgnorePatternWhitespace);
            ...
            foreach (Match match in matches)
            {
                if (!String.IsNullOrEmpty(match.Value))
                {
                    ...
                    string coef_s = match.Groups["coef"].Value;
            ...
            // нужно находить максимальную степень и ставить сюда
        ...
        {
            writer.WriteValue(ToString(null, CultureInfo.InvariantCulture, "x").Replace(" ", ""));
        }
